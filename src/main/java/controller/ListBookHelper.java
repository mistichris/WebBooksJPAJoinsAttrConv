...
public class ListBookHelper {
	...
	public List<BookListItems> showAllBooks(){
		EntityManager em = emfactory.createEntityManager();											//creates a new instance of the Entity Manager
		List<BookListItems> allBooks = em.createQuery("SELECT i FROM ListBook i").getResultList();		//retrieves all instances of the item from the table
		return allBooks;
		}
	...
	public void deleteBook(BookListItems toDelete) {
		...
		em.getTransaction().begin();
		TypedQuery<BookListItems> typedQuery = em.createQuery("Select li from ListBook li where li.id = :selectedId", 
				BookListItems.class);
...
//		typedQuery.setParameter("selectedBook", toDelete.getBook());				//toDelete ListBook object created from StartProgram and getBook method from ListBook
		...
		BookListItems result = typedQuery.getSingleResult();					//creating a ListBook object for the results
	...
	public List<BookListItems> searchForBookByAuthor(String authorName) {			//from StartProgram editABook method
		EntityManager em = emfactory.createEntityManager();			//new instance of Entity Manager Factory
		em.getTransaction().begin();
		TypedQuery<BookListItems> typedQuery = em.createQuery("select li from ListBook li where li.author = :selectedAuthor", BookListItems.class);		//returns an object of a given parameter to a new list titled 'typedQuery'
	...
	public List<BookListItems> searchForBookByTitle(String bookTitle) {			//from StartProgram editABook method
		EntityManager em = emfactory.createEntityManager();			//new instance of Entity Manager Factory
		em.getTransaction().begin();
		TypedQuery<BookListItems> typedQuery = em.createQuery("select li from ListBook li where li.book = :selectedBook", BookListItems.class);		//returns an object of a given parameter to a new list titled 'typedQuery'
