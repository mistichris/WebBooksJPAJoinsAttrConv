...
//				lbh.cleanUp();
	...
	private static void editABook() { // edit an author constructor
		...
		if (!foundBooks.isEmpty()) {
			...
			for (BookListItems l : foundBooks) {
				System.out.println(l.getId() + " : " + l.toString());
			}
			System.out.print("Which ID to edit: ");
			int idToEdit = in.nextInt();

			BookListItems toEdit = lbh.searchForBookById(idToEdit); // new instance ListBook object using the ListBookHelper
																// method searchForB
	...
	private static void viewTheList() { // Show all database/table authors
		List<BookListItems> allBooks = lbh.showAllBooks(); // calls method showAllBooks in the ListBookHelper class
		for (BookListItems singleBook : allBooks) {
